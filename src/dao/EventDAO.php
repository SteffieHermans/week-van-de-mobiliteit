...
class EventDAO extends DAO {
  ...
  public function searchUpToThree($conditions = array(), $sequenceList = '') {
    $sql = "SELECT DISTINCT
      ma3_auto_events.*
      FROM `ma3_auto_events`
      LEFT OUTER JOIN `ma3_auto_events_organisers` ON ma3_auto_events.id = ma3_auto_events_organisers.event_id
    ...
    if(!empty($sequenceList)) {
      $sql .= ' AND `postal` IN(' . $sequenceList . ')';
    }
    $sql .= ' LIMIT 3';
    $stmt = $this->pdo->prepare($sql);
    $stmt->execute($conditionParams);
  ...
  public function selectById($id) {
    ...
    $tagsByEventId = $this->_getTagsForEventId($id);
    $organisersByEventId = $this->_getOrganisersForEventId($id);
    $result['tags'] = array();
    if(!empty($tagsByEventId[$id])) {
      $result['tags'] = $tagsByEventId[$id];
    }
    $result['organisers'] = array();
    if(!empty($organisersByEventId[$id])) {
      $result['organisers'] = $organisersByEventId[$id];
    }
  ...
  private function _getOrganisersForEventId($eventId) {
    $organisersByEventId = array();
    $sql = "SELECT
      ma3_auto_organisers.*,
      ma3_auto_events_organisers.event_id
      FROM `ma3_auto_organisers`
      RIGHT OUTER JOIN `ma3_auto_events_organisers` ON ma3_auto_events_organisers.organiser_id = ma3_auto_organisers.id
      WHERE ma3_auto_events_organisers.event_id = :eventId
    ";
    $stmt = $this->pdo->prepare($sql);
  ...
  private function _getTagsForEventId($eventId) {
    $tagsByEventId = array();
    $sql = "SELECT
      ma3_auto_tags.*,
      ...
      WHERE ma3_auto_events_tags.event_id = :eventId
    ";
    $stmt = $this->pdo->prepare($sql);
    $stmt->bindValue(':eventId', $eventId);
    $stmt->execute();
